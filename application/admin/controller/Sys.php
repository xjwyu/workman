...
class Sys extends Base{
     ...
     */
	public function sys(){
	    //echo APP_PATH;   //E:\EPP\yu\public/../application/
		//主题
		$tpls=Options::themes();
		$this->assign('templates',$tpls);
		$sys=Options::get_options('site_options',$this->lang);
		$map=Options::map($this->lang); //获取公司所在的经纬度
	...
	public function runsys(){
	    if (!request()->isAjax()){
	        ...
	        $options=input('post.options/a');   //接收所有表单数据并转化为数组   /a是转化为数组的意思
	        //dump($options);
	        $img_url='';
	        if ($checkpic!=$oldcheckpic){
	            ...
	            if(!empty($file)){
	                if(config('storage.storage_open')){
	                    ...
	                    $info = $file->validate($validate)->rule('uniqid')->move(config('upload_path') . DS . date('Y-m-d'));   //rule方法动态添加规则   移动到服务器的上传目录 并且使用uniqid规则
	                    //var_dump($info);exit;
	                    if($info) {
	...
	public function runemail(){
		if (!request()->isAjax()){
			...
			if(empty($options['email_open'])){
				$options['email_open'] = 0;
			}
			//dump($options);exit();
			$rst=Options::set_options($options,'email_options',$this->lang);
			if($rst!==false){
				cache("email_options", null);
				cache("email_options_".$this->lang,null);
	...
	public function runactive(){
		if (!request()->isAjax()){
			...
			if($rst!==false){
				cache("active_options_".$this->lang,null);
	...
	public function export($tables = null, $id = null, $start = null){
		if(request()->isPost() && !empty($tables) && is_array($tables)){ //初始化
			//读取备份配置
			$config = array(
					'path'     => realpath(config('db_path')) . DS,  //函数返回绝对路径
					'part'     => config('db_part'),
			...
			if(is_file($lock)){
				return json(array('code'=>-1,'msg'=>"检测到有一个备份任务正在执行，请稍后再试！"));
	...
	public function import(){
		...
		$path = realpath($path);  //返回绝对路径
		$flag = \FilesystemIterator::KEY_AS_FILENAME;
		$glob = new \FilesystemIterator($path,$flag);
		$list = array();
		foreach ($glob as $name => $file) {			
			if(preg_match('/^\d{8,8}-\d{6,6}-\d+\.sql(?:\.gz)?$/', $name)){
				$name = sscanf($name, '%4s%2s%2s-%2s%2s%2s-%d');//解析字符串
	...
	public function admin_rule_list()
	{
		...
		$level=input('level',0);
		$id_str=input('id','pid');
		$admin_rule=Db::name('auth_rule')->where('pid',$pid)->where('is_delete',0)->order('sort')->select();
		$admin_rule_all=Db::name('auth_rule')->order('sort')->where('is_delete',0)->select();
	...
	public function admin_rule_runadd()
	{
		if(!request()->isAjax()){
			...
			$pid=Db::name('auth_rule')->where(array('id'=>input('pid')))->where('is_delete',0)->field('level')->find();
			$level=$pid['level']+1;
			$name=input('name');
	...
	{
		//全部规则
		$admin_rule_all=Db::name('auth_rule')->where('is_delete',0)->order('sort')->select();
		$arr = menu_left($admin_rule_all);
		$this->assign('admin_rule',$arr);
		//待编辑规则
	...
	public function admin_rule_del(){
	    $pid=input('id');
	    $arr=Db::name('auth_rule')->where('is_delete',0)->select();
	    $tree=new \Tree();
	    $tree->init($arr,['parentid'=>'pid']);
	    $arrTree=$tree->get_childs($arr,$pid,true,true);
	    if($arrTree){
	        $map = array('is_delete'=>1);
	        $rst=Db::name('auth_rule')->where('id','in',$arrTree)->update($map);
	        if($rst!==false){
	            Cache::clear();
