...
class Admin extends Model{
	...
	public function authGroups()
	{
		...
		//关联表（角色表auth_group） 中间表(管理员-角色auth_group_access)  关联表在中间表（auth_group）的字段名  当前模型在中间表的字段名
	}
	
	/**
	 * 分页搜索列表
	 * @param unknown $search 搜索条件
	 * @return unknown
	 */
	public function search($search)
	{
		...
		if($search['search_name']){
			$map['admin_username']= array('like',"%".$search['search_name']."%");
		}
		$list = $this->where($map)->order('admin_id')->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
		return $list;
	}
	/**
	 * 用户登录
	...
	public function auto_login($user, $rememberme = false){
		...
		$member=Db::name('member_list')->where('member_list_id',$auth['member_id'])->where('is_delete',0)->find();
	...
	public static function add($admin_username,$admin_pwd_salt='',$admin_pwd,$admin_email='',$admin_tel='',$admin_open=0,$admin_realname='',$group_id=1)
	{
		...
		if($admin){
			//添加管理组
			$admin->authGroups()->save($group_id);  //关联新增 管理员-角色的数据 auth_group_access
...
// 			Db::name('auth_group_access')->insert($accdata);
		...
		if($rst!==false){
			...
			if($access){
				...
				if($access['group_id']!=$data['group_id']){
					self::get($data['admin_id'])->authGroups()->detach([$access['group_id']]);	//先删除				
					self::get($data['admin_id'])->authGroups()->attach([$data['group_id']]);  //再增加 REPLACE INTO `yf_auth_group_access` (`uid` , `group_id`) VALUES (4 , 1)
	...
	{
		//对应会员ID
		$member_id = self::where('admin_id',$id)->value('member_id');
		//self::destroy($id);//直接删除
		//自己的软删除
		self::where('admin_id',$id)->setField('is_delete',1);
		//删除对应会员
		if($member_id){
			//Db::name('member_list')->delete($member_id);  //直接删除
			Db::name('member_list')->where('member_list_id',$member_id)->setField('is_delete',1);
		}
		//删除中间表的数据
		$rst=Db::name('auth_group_access')->where('uid',$id)->delete();
		if($rst!==false){
			return true;
		}else{
			return false;
